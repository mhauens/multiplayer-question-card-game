...
- For gameplay or socket changes, explicitly call out whether server rules, handler permissions, and client affordances still appear aligned.
- For variant, extension, or theme changes, explicitly call out whether catalog wiring and presentation stay aligned.
...
- For gameplay, lobby, reconnect, or card-flow changes, mention the two-browser smoke test when it has not been covered.
- For variant, extension, or theme changes, mention host-side variant switching and extension compatibility verification when relevant.
